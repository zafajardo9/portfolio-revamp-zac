...
export default function GitHubActivity() {
  ...
  const renderHeatmap = () => {
    ...
    return (
      <div className="overflow-x-auto py-6">
        <div
          className="flex flex-nowrap mx-auto gap-[1px]"
          style={{ maxWidth: "900px" }}
        >
          {weeks.map((week, weekIndex) => (
            <div key={`week-${weekIndex}`} className="flex flex-col gap-[1px]">
              {week.map((day) => (
                <div
                  key={`day-${day.date}`}
                  className="w-3 h-3 hover:ring-1 hover:ring-offset-1 transition-all border border-secondary/20"
  ...
  return (
    <section id="github-activity" className="py-20">
      <div className="mb-8">
        <h2 className="text-3xl font-bold tracking-tight text-secondary mb-2">
          GitHub Activity
        </h2>
